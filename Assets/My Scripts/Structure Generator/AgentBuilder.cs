...
public class AgentBuilder {
    ...
    public AgentBuilder(float minDistance, float flatnessThreshold, int numberOfPoints) {
    ...
    public List<Vector3> PickPoints(Planet planet) {
        ...
        foreach (TerrainFace face in planet.terrainFaces) {
            ...
            for (int i = 0; i < vertices.Length; i++) {
                ...
                if (IsFlat(normals[i]) && IsFarEnough(points, pointOnSurface)) {
                    points.Add(pointOnSurface);
